...
var DEFAULT_MOUSE_WHEEL_THRESHOLD = 10;
var DEFAULT_DRAG_THRESHOLD = 0.08;
var DEFAULT_CONFIG = {
  autoplay: 0,
...
function useWheel(options) {
  ...
  const handleScroll = (event) => {
    ...
    const threshold = typeof config.mouseWheel === "object" ? (_a = config.mouseWheel.threshold) !== null && _a !== void 0 ? _a : DEFAULT_MOUSE_WHEEL_THRESHOLD : DEFAULT_MOUSE_WHEEL_THRESHOLD;
  ...
  setup(props, { slots, emit, expose }) {
    ...
    function updateSlidesRectSize({ widthMultiplier, heightMultiplier }) {
      slidesRect.value = slides.map((slide) => {
        var _a2;
        const rect = (_a2 = slide.exposed) === null || _a2 === void 0 ? void 0 : _a2.getBoundingRect();
        return {
    ...
    function updateViewportRectSize({ widthMultiplier, heightMultiplier }) {
      var _a2;
      const rect = ((_a2 = viewport.value) === null || _a2 === void 0 ? void 0 : _a2.getBoundingClientRect()) || { width: 0, height: 0 };
      viewportRect.value = {
        width: rect.width * widthMultiplier,
        height: rect.height * heightMultiplier
      };
    ...
    const onDrag = ({ deltaX, deltaY, isTouch }) => {
      ...
      const threshold = isTouch ? typeof config.touchDrag === "object" ? (_b = (_a2 = config.touchDrag) === null || _a2 === void 0 ? void 0 : _a2.threshold) !== null && _b !== void 0 ? _b : DEFAULT_DRAG_THRESHOLD : DEFAULT_DRAG_THRESHOLD : typeof config.mouseDrag === "object" ? (_d = (_c = config.mouseDrag) === null || _c === void 0 ? void 0 : _c.threshold) !== null && _d !== void 0 ? _d : DEFAULT_DRAG_THRESHOLD : DEFAULT_DRAG_THRESHOLD;
    ...
    const clonedSlidesOffset = computed(() => {
      ...
      if (isAuto.value) {
        return slidesRect.value.slice(-1 * clonedSlidesCount.value.before).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) * -1;
      }
    ...
    const snapAlignOffset = computed(() => {
      ...
      if (isAuto.value) {
        ...
        return getSnapAlignOffset({
          slideSize: (_a2 = slidesRect.value[slideIndex]) === null || _a2 === void 0 ? void 0 : _a2[dimension.value],
    ...
    const scrolledOffset = computed(() => {
      ...
      if (isAuto.value) {
        if (currentSlideIndex.value < 0) {
          output = slidesRect.value.slice(currentSlideIndex.value).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) * -1;
        } else {
          output = slidesRect.value.slice(0, currentSlideIndex.value).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0);
        }
        output -= snapAlignOffset.value;
        if (!config.wrapAround) {
          const maxSlidingValue = slidesRect.value.reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) - viewportRect.value[dimension.value] - config.gap;
    ...
    const visibleRange = computed(() => {
      ...
      let minIndex = 0;
      {
        ...
        while (accumulatedSize <= offset) {
          ...
          accumulatedSize += ((_a2 = slidesRect.value[normalizedIndex]) === null || _a2 === void 0 ? void 0 : _a2[dimension.value]) + config.gap;
          index++;
        }
        minIndex = index - 1;
      }
      let maxIndex = 0;
      {
        ...
        let accumulatedSize = 0;
        if (index < 0) {
          accumulatedSize = slidesRect.value.slice(0, index).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) - Math.abs(scrolledOffset.value + clonedSlidesOffset.value);
        } else {
          accumulatedSize = slidesRect.value.slice(0, index).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) - Math.abs(scrolledOffset.value);
        }
        while (accumulatedSize < viewportRect.value[dimension.value]) {
          const normalizedIndex = (index % slides.length + slides.length) % slides.length;
          accumulatedSize += ((_b = slidesRect.value[normalizedIndex]) === null || _b === void 0 ? void 0 : _b[dimension.value]) + config.gap;
          index++;
        }
        maxIndex = index - 1;
      }
    ...
    const trackTransform = computed(() => {
      ...
      if (!config.wrapAround && config.preventExcessiveDragging) {
        ...
        if (isAuto.value) {
          ...
          maxSlidingValue = (slidesCount.value - Number(config.itemsToShow)) * effectiveSlideSize.value;
        }
    ...
    const carouselStyle = computed(() => ({
      ...
      "--vc-transition-duration": isSliding.value ? toCssValue(config.transition, "ms") : void 0
    }));
    ...
    return () => {
      ...
      for (let slide = isPaginated.value ? 0 : carousel.minSlide; slide <= (isPaginated.value ? pageCount.value - 1 : carousel.maxSlide); slide++) {
        ...
        const button = h("button", {
          ...
          onClick: () => carousel.nav.slideTo(isPaginated.value ? Math.floor(slide * +carousel.config.itemsToShow + offset.value) : slide)
        });
...
var Slide = defineComponent({
  ...
  setup(props, { attrs, slots, expose }) {
    ...
    const isVisible = computed(() => currentIndex.value >= carousel.visibleRange.min && currentIndex.value <= carousel.visibleRange.max);
